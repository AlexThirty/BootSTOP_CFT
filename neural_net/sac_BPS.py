            ...
            if self.env.reward_improved:
                ...
                if output_order == 'Reversed':
                    if self.integral_mode == 2:
                        file_output = np.concatenate((current_location, current_reward, self.env.current_crossing, self.env.current_int_constr_1, self.env.current_constr_2, array_index), axis=None)
                    elif self.integral_mode == 1:
                        file_output = np.concatenate((current_location, current_reward, self.env.current_crossing, self.env.current_int_constr_1, array_index), axis=None)
                    else:
                        file_output = np.concatenate((current_location, current_reward, self.env.current_crossing, array_index), axis=None)
                else:
                    if self.integral_mode == 2:
                        file_output = np.concatenate((array_index, current_reward, self.env.current_crossing, self.env.current_int_constr_1, self.env.current_int_constr_2, current_location), axis=None)
                    elif self.integral_mode == 1:
                        file_output = np.concatenate((array_index, current_reward, self.env.current_crossing, self.env.current_int_constr_1, current_location), axis=None)
                    else:
                        file_output = np.concatenate((array_index, current_reward, self.env.current_crossing, current_location), axis=None)

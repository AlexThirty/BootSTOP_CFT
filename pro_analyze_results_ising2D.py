...
for i in range(delta_tries):
    ...
    for el in reversed(orderer):
        output_to_file(file_name=join('pro_analyzed_'+suffix,'pro_res_deltas'+str(i)+'.csv'), output=np.concatenate(([el], [coll[el]], deltas_coll[el], lambdas_coll[el])))
...
plt.savefig(join('pro_analyzed_'+suffix, 'rew_for_deltas.jpg'), dpi=300)
plt.close()
...
plt.savefig(join('pro_analyzed_'+suffix, 'rew_for_deltas_mean.jpg'), dpi=300)
plt.close()
...
plt.savefig(join('pro_analyzed_'+suffix, 'lambda_error_best.jpg'), dpi=300)
plt.close()
...
plt.xlabel('Number of deltas fixed')
plt.ylabel('Mean of lambda relative errors')
plt.savefig(join('pro_analyzed_'+suffix, 'lambda_error_mean.jpg'), dpi=300)
plt.close()
...
plt.savefig(join('pro_analyzed_'+suffix, 'lambda_error_best_sing.jpg'), dpi=300)
plt.close()
...
plt.yscale('log')
plt.legend(fontsize=5)
plt.savefig(join('pro_analyzed_'+suffix, 'lambda_error_mean_sing.jpg'), dpi=300)
plt.close()
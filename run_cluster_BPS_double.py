...
if __name__ == '__main__':
    ...
    OPE_fix=1
    integral_mode = 2
    res_path_1 = f'/data/trenta/results_BPS_1fix_g005'
    res_path_3 = f'/data/trenta/results_BPS_3fix_g005'
    res_path_steps = f'/data/trenta/results_BPS_{OPE_fix}fix_g005_steps'
    if not os.path.exists(res_path_1):
        os.makedirs(res_path_1)
    if not os.path.exists(res_path_3):
        os.makedirs(res_path_3)
    ...
    for i in range(args.runs_per_args):
        ...
        array_index = start_run + i

        # form the file_name where the code output is saved to
        file_name = os.path.join(res_path_1, params.filename_stem + str(array_index) + '.csv')
    ...
    for i in range(args.runs_per_args):
        ...
        # form the file_name where the code output is saved to
        file_name = os.path.join(res_path_3, params.filename_stem + str(array_index) + '.csv')
        file_name_steps = os.path.join(res_path_steps, params.filename_stem + str(array_index) + '_steps.csv')
        output = str(array_index)
        utils.output_to_file(file_name=file_name, output=output)
        # determine initial starting point in the form needed for the soft_actor_critic function
